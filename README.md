...
Use `verify.py` to estimate the dominant left and right channel frequencies in one
or more audio files and report the difference as the beat frequency.

```bash
python3 verify.py sine-200-10hz-300s.mp3
python3 verify.py *.mp3
```

This is a simple dominant-frequency check, so it works best for the clean tone
